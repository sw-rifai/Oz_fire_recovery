...
d_rf <- d_rf %>% mutate(x = round(x.x*10)/10,
  y=round(y.x*10)/10) %>% 
  as.data.table()
...
grid0
gbm_gridperf0 <- h2o.getGrid(grid_id = 'grid_17',
                             sort_by = "rmse",
                             decreasing = F)
gbm_gridperf0@summary_table

h2o.performance(h2o.getModel(gbm_gridperf0@model_ids[[1]]),newdata=valid)
h2o.performance(h2o.getModel(gbm_gridperf0@model_ids[[1]]),newdata=as.h2o(d_out))
# RMSE 315 (no species)
