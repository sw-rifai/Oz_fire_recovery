...
# ML predictions ---------------------
out <- arrow::read_parquet("../data_general/proc_data_Oz_fire_recovery/predicted_nobs80-species-distribution-ala-mq_XGBoost_grid__1_AutoML_20210705_104746_model_8_2021-07-05 14:24:06.parquet")
...
for(i in 1:length(idx)){
 ...
 p_out <-  ggplot(data=mout[species%in%vec_names[idx[i]:idx[i+1]]],
  ...
  ggsave(p_out, 
       filename=paste0('figures/euc_sdm_v0.2/',im_name), 
         width=16*2,
         height=9*2,
         units='cm',
         dpi=350)
