...
dat[,`:=`(kndvi = tanh(ndvi**2))] # calculate kNDVI
...
norms <- dat %>% lazy_dt() %>% 
  ...
  group_by(id,year) %>% 
  summarize(kn_yr = mean(skndvi,na.rm=TRUE)) %>% 
  ungroup() %>% 
  group_by(id) %>% 
...
norms_monthly <- dat %>% lazy_dt() %>% 
  group_by(month,id) %>% 
  summarize(kn_u = mean(skndvi,na.rm=TRUE), 
            kn_sd = sd(skndvi,na.rm=TRUE)) %>% 
  ungroup() %>% 
  as.data.table()
