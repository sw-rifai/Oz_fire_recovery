...
c_matrange <- clim[,.(trange = tmax_u-tmin_u), by=.(x,y,month)] %>% 
  .[,.(matrange = mean(trange,na.rm=T)), by=.(x,y)]

clim_ma <- clim[,.(x,y,map,mapet,mappet,matmax,matmin,mavpd15)] %>% unique()
clim_ma <- merge(clim_ma, c_matrange, by=c('x','y'))

lut_seasons <- data.table(month=1:12)[,season := case_when(month%in%c(12,1,2)~"DJF",
                                            month%in%c(3,4,5)~"MAM",
                                            month%in%c(6,7,8)~"JJA",
                                            month%in%c(9,10,11)~"SON")]
c_trange <- clim[,.(trange_u = tmax_u-tmin_u), by=.(x,y,month)]

clim <- merge(clim,c_trange,by=c("x","y","month"))
clim <- merge(clim, lut_seasons,by='month')
clim <- clim[,.(precip_u = sum(precip_u), 
        tmax_u = mean(tmax_u), 
        tmin_u = mean(tmin_u), 
        trange_u = mean(trange_u),
...
clim_trange <- dcast(clim[,.(x,y,season,trange_u)], x+y~paste0("trange_season_",season),  value.var = 'trange_u')
...
clim_wide <- merge(clim_wide, clim_trange, by=c('x','y'))
...
# Soil variables - reduced -----------------------------------
s_soil <- read_stars("../data_general/Oz_misc_data/SLGA_500m_SE_coastal.tif",                      proxy = F) 
st_crs(s_soil) <- st_crs(4326)
# s_soil <- split(s_soil,f = 'band')
names_soil <- st_get_dimension_values(s_soil,3)
...
library(future)
plan(multisession)
s_bdw <- st_apply(s_soil[,,,str_which(names_soil,"BDW_")],
  c("x","y"),
  FUN=mean,
  FUTURE=T) 
names(s_bdw) <- 'bdw'
s_clay <- st_apply(s_soil[,,,str_which(names_soil,"CLY_")],
  c("x","y"),
  FUN=mean,
  FUTURE=T) 
names(s_clay) <- 'clay'
s_silt <- st_apply(s_soil[,,,str_which(names_soil,"SLT_")],
  c("x","y"),
  FUN=mean,
  FUTURE=T) 
names(s_silt) <- 'silt'
s_sand <- st_apply(s_soil[,,,str_which(names_soil,"SND_")],
  c("x","y"),
  FUN=mean,
  FUTURE=T) 
names(s_sand) <- 'sand'
s_pH <- st_apply(s_soil[,,,str_which(names_soil,"pHc_")],
  c("x","y"),
  FUN=mean,
  FUTURE=T) 
names(s_pH) <- 'ph'
s_der <- s_soil[,,,str_which(names_soil,'DER_')] %>% 
  set_names('der')
s_des <- s_soil[,,,str_which(names_soil,'DES_')] %>% 
  set_names('des')

plan(sequential)
s_soil <- c(s_bdw,s_clay,s_silt,s_sand,s_pH,adrop(s_der),adrop(s_des))



# Tree cover 2000 ---------------------------------------------------------
s_tc2000 <- read_stars("../data_general/proc_data_Oz_fire_recovery/GFC-treecover2000_500m_SE_coastal.tif") %>% 
  set_names("treecover2000")

# Tree height GEDI v27 ---------------------------------------------------------
s_th <- read_stars("../data_general/proc_data_Oz_fire_recovery/GEDI-AUS-v27_500m_SE_coastal.tif") %>% 
  set_names("tree-height")

# Crop type ---------------------------------------------------------------
s_crop <- read_stars("../data_general/proc_data_Oz_fire_recovery/GFSAD1000_500m_SE_coastal.tif") %>% 
  set_names('crop-type')
...
vv <- st_drop_geometry(st_extract(c(s_tc2000), at=dat))
dat <- bind_cols(dat,vv)

vv <- st_drop_geometry(st_extract(c(s_crop), at=dat))
dat <- bind_cols(dat,vv)

vv <- st_drop_geometry(st_extract(c(s_th), at=dat))
dat <- bind_cols(dat,vv)
...
fdat <- fdat[is.na(ph)==F]  
fdat <- fdat[is.na(des)==F]  
...
fdat2 <- fdat2[,-c("geometry",
  # "year",
  "taxon","vc_name","vc_code")]
dim(fdat2)
...
h2o.performance(top_mod,valid)@metrics$mean_per_class_error #0.153 # 0.2(80 forest)
h2o.performance(top_mod,valid)@metrics$logloss # 0.614
h2o.performance(top_mod,valid)@metrics$MSE
h2o.performance(top_mod,valid)@metrics$RMSE
# MSE 0.21
# RMSE 0.459
...
vv <- st_drop_geometry(st_extract(c(rhnd), at=pc))
pc <- bind_cols(pc,vv)
vv <- st_extract(s_tc2000, pc)
pc <- bind_cols(pc, st_drop_geometry(vv))
vv <- st_extract(s_th, pc)
pc <- bind_cols(pc, st_drop_geometry(vv))
vv <- st_extract(s_crop, pc)
pc <- bind_cols(pc, st_drop_geometry(vv))
pc <- bind_cols(st_drop_geometry(pc), 
          st_coordinates(vv) %>% as.data.table() %>% set_names(c("x","y")))
pc <- na.omit(pc)
