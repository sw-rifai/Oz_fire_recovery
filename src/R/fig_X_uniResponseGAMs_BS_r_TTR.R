...
p_bs_1 <-  draw(bs1,select="frac_pf_slai_anom_3mo",partial_match = T) +
  ...
  labs(
    ...
    title=NULL,
       ...
       x = expression(paste(frac("Pre-fire"~LAI["3 mo. anom"],
         LAI["normal"]))))+
...
p_bs_2 <- draw(bs1,select="frac_pf_slai_anom_12mo",partial_match = T) +
  ...
  geom_vline(xintercept=0,lty=2,col='grey') + 
    ...
    title=NULL,
         x = expression(paste(frac("Pre-fire"~LAI["12 mo. anom"],
           LAI["normal"]))))+
  scale_x_continuous(expand=c(0,0))+
  theme_linedraw()+
...
p_bs_3 <- draw(bs1,select="precip_anom_3mo",partial_match = T) +
  ...
  geom_vline(xintercept=0,lty=2,col='grey') + 
    labs(
      ...
      # frac(LAI["post-fire"],LAI["normal"]))),
    title=NULL,
         x = expression(paste(frac("Pre-fire"~P["3 mo. anom"],
           P["normal"]))))+
  scale_x_continuous(expand=c(0,0))+
  theme_linedraw()+
...
p_r1_3 <- draw(r1,select="post_precip_anom_frac",partial_match = T) +
  ...
  geom_vline(xintercept=0,lty=2,col='grey') + 
    labs(
      ...
      # frac(LAI["post-fire"],LAI["normal"]))),
    title=NULL,
         x = expression(paste(frac("Post-fire"~P["12 mo. anom"],
           P["Mean Annual"]))))+
  scale_x_continuous(expand=c(0,0))+
  theme_linedraw()+
...
p_r1_4 <- draw(r1,select="s(log(mappet))",partial_match = F) +
  ...
  geom_vline(xintercept=0,lty=2,col='grey') + 
    ...
    title=NULL,
         x = expression(paste(Log~bgroup("(","Mean Annual"~frac(~P,
           PET),")"))))+
  scale_x_continuous(expand=c(0,0))+
  theme_linedraw()+
...
p_t1_1 <- draw(t1,select="bs",partial_match = T) +
  ...
  # geom_vline(xintercept=0,lty=2,col='grey') + 
    labs(
      x= expression(paste(frac(LAI["post-fire"],LAI["normal"]))),
      y = expression(paste("Effect on ",
        "TTR "~(days)
        )),
    title=NULL,
...
p_t1_2 <-  draw(t1,select="frac_p12_anom",partial_match = T) +
  ...
  #   geom_line(colour = "black", size = 1.5) +
    labs(
      x=expression(paste("Pre-fire"~frac(P["12 mo. anom."],P["Mean Annual"]))),
      y = expression(paste("Effect on ",
        "TTR "~(days)
        )),
    title=NULL,
...
((p_bs_4|p_bs_5|p_bs_6)+plot_layout(ncol=3))  +
 ...
 title = 'Partial drivers of Burn Severity')) 
ggsave(p_top,
...
((p_r1_4|p_r1_5|p_r1_6)+plot_layout(ncol=3))+
 ...
 title = 'Partial drivers of Recovery Rate'))
ggsave(p_mid,
...
title = 'Partial drivers of Time to Recover')
ggsave(p_bot,
