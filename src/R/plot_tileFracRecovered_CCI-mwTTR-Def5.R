...
dat <- dat[,.(x,y,date,id, cci,cci_u,fire_doy)]
gc(full=TRUE)
norms <- dat %>% lazy_dt() %>% 
  mutate(year=year(date)) %>% 
  group_by(id,year) %>% 
  summarize(cci_yr = mean(cci,na.rm=TRUE)) %>% 
  ungroup() %>% 
  group_by(id) %>% 
  summarize(macci = mean(cci_yr,na.rm=TRUE), 
            cci_yr_sd = sd(cci_yr,na.rm=TRUE)) %>% 
  ungroup() %>% 
  as.data.table()
...
rm(norms)
gc(full=TRUE)
dat[,`:=`(cci_anom = cci-cci_u)]
dat <- dat[order(x,y,date)][,cci_anom_3mo := frollmean(cci_anom,
                                                       n = 3,fill = NA,align='right'), 
                            by=.(x,y)]
gc(full=TRUE)
dat <- dat[order(x,y,date)][,cci_anom_12mo := frollmean(cci_anom,
                                                        n = 12,fill = NA,align='right'), 
                            by=.(x,y)]
gc(full=TRUE)
dat <- dat[order(x,y,date)][,cci_anom_24mo := frollmean(cci_anom,
                                                        n = 24,fill = NA,align='right'), 
                            by=.(x,y)]
gc(full=TRUE)
dat <- dat[order(x,y,date)][,cci_anom_36mo := frollmean(cci_anom,
...
d_min_cci <- dat1 %>% 
  ...
  filter(is.na(cci_anom)==FALSE) %>% 
  group_by(id) %>% 
  summarize(min_cci_anom = min(cci_anom,na.rm=TRUE)) %>% 
  ungroup() %>% 
  as.data.table()
gc(full=TRUE)
dat1 <- merge(dat1,d_min_cci,by='id')
gc(full=TRUE)
...
fn_ttr5 <- function(din){
  ttr <- din[days_since_fire>=365][cci_anom_12mo>= -0.25*cci_yr_sd]$days_since_fire
  ttr <- fn_min(ttr)
  din$ttr5_cci <- ttr
  din$pre_fire_cci_anom_3mo <- din[date == floor_date(date_fire1-1,'month')]$cci_anom_3mo[1]
  din$pre_fire_cci_anom_12mo <- din[date == floor_date(date_fire1-1,'month')]$cci_anom_12mo[1]
  din$pre_fire_cci_anom_24mo <- din[date == floor_date(date_fire1-1,'month')]$cci_anom_24mo[1]
  din$pre_fire_cci_anom_36mo <- din[date == floor_date(date_fire1-1,'month')]$cci_anom_36mo[1]
...
arrow::write_parquet(out[date==date_fire1][,.(x,y,id,date_fire1,fire_doy,
                                              ...
                                              macci,
                                              cci_anom_3mo,
                                              pre_fire_cci_anom_3mo,
                                              pre_fire_cci_anom_12mo,
                                              pre_fire_cci_anom_24mo,
                                              pre_fire_cci_anom_36mo, 
                                              ttr5_cci)], 
                     sink = paste0("../data_general/proc_data_Oz_fire_recovery/fit_cci_ttrDef5_preBS",Sys.time(),".parquet"))
...
# FASTER Method -----------------------------------------------------------
out <- arrow::read_parquet("../data_general/proc_data_Oz_fire_recovery/fit_cci_ttrDef5_preBS2021-04-19 11:39:00.parquet")
...
d3 %>% write_parquet(.,
         sink="../data_general/proc_data_Oz_fire_recovery/cumulativeRecovery_cci_ttrDef5_preBS.parquet")
...
ggsave(filename = 'figures/figure_cumulativeRecovered_byYear_TTR-Def5-cci.png',
       width=15,
       height=10,
       units='cm',
       dpi=350)
