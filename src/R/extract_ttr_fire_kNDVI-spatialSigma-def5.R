...
d_sigma <- dat %>% lazy_dt() %>% 
  group_by(id) %>% 
  mutate(diff_nr = 0.5*(red+nir)) %>% 
  group_by(id) %>% 
  summarize(sigma = mean(diff_nr,na.rm=TRUE)) %>% 
  ungroup() %>% 
  as.data.table()
  
dat <- merge(dat, d_sigma, by='id')
setkey(dat,NULL) #!!! IMPORTANT
dat[, `:=`(ndvi = (nir - red)/(red + nir))]
dat[,`:=`(kn_simple = tanh(ndvi**2))] # calculate simplified kNDVI
dat[,`:=`(knr = exp( -(nir-red)**2 / (2*sigma**2)))]
dat[,`:=`(kndvi = (1-knr)/(1+knr))]
dat[,`:=`(kn=kndvi)]

# # Fn: Smooth data with Whittaker filter -----------------------------
# smooth_vi <- function(din,vi='kndvi'){
#   din <- din[order(date)]
...
# system.time(dat <- dat[,smooth_vi(.SD), by=.(x,y,id)]) # ~5 mins
# setnames(dat,"svi","skndvi")
# gc(full=TRUE)
...
norms <- dat %>% lazy_dt() %>% 
  ...
  group_by(id,year) %>% 
  summarize(kn_yr = mean(kn,na.rm=TRUE)) %>% 
  ungroup() %>% 
  group_by(id) %>% 
  summarize(makn = mean(kn_yr,na.rm=TRUE), 
            kn_yr_sd = sd(kn_yr,na.rm=TRUE)) %>% 
  ungroup() %>% 
  as.data.table()

norms_monthly <- dat %>% lazy_dt() %>% 
  group_by(month,id) %>% 
  summarize(kn_u = mean(kn,na.rm=TRUE), 
            kn_sd = sd(kn,na.rm=TRUE)) %>% 
  ungroup() %>% 
  as.data.table()
...
setkey(dat,NULL)
dat[,`:=`(kn_anom = kn-kn_u)]
dat <- dat[order(x,y,date)][,kn_anom_3mo := frollmean(kn_anom,
                                                         n = 3,fill = NA,align='right'), 
                            by=.(x,y)]
gc(full=TRUE)
dat <- dat[order(x,y,date)][,kn_anom_12mo := frollmean(kn_anom,
                                    n = 12,fill = NA,align='right'), 
                            by=.(x,y)]
gc(full=TRUE)
dat <- dat[order(x,y,date)][,kn_anom_24mo := frollmean(kn_anom,
                                                            n = 24,fill = NA,align='right'), 
                            by=.(x,y)]
gc(full=TRUE)
dat <- dat[order(x,y,date)][,kn_anom_36mo := frollmean(kn_anom,
...
dat %>% write_parquet(., "../data_general/proc_data_Oz_fire_recovery/mcd43_se_coastal_kndvi_2001-2020.parquet")
...
fn_ttr5 <- function(din){
  ttr <- din[days_since_fire>=365][kn_anom_12mo>= -0.25*kn_yr_sd]$days_since_fire
  ttr <- fn_min(ttr)
  din$ttr5 <- ttr
  din$pre_fire_vi_anom_3mo <- din[date == floor_date(date_fire1-1,'month')]$kn_anom_3mo[1]
  din$pre_fire_vi_anom_12mo <- din[date == floor_date(date_fire1-1,'month')]$kn_anom_12mo[1]
  din$pre_fire_vi_anom_24mo <- din[date == floor_date(date_fire1-1,'month')]$kn_anom_24mo[1]
  din$pre_fire_vi_anom_36mo <- din[date == floor_date(date_fire1-1,'month')]$kn_anom_36mo[1]
...
out <- out %>% lazy_dt() %>% rename(ttr5_kn = ttr5, 
                                    pre_fire_kn_anom_3mo = pre_fire_vi_anom_3mo,
                                    pre_fire_kn_anom_12mo = pre_fire_vi_anom_12mo,
                                    pre_fire_kn_anom_24mo = pre_fire_vi_anom_24mo,
                                    pre_fire_kn_anom_36mo = pre_fire_vi_anom_36mo,
                                     ) %>% as.data.table()
arrow::write_parquet(out[date==date_fire1], 
      sink = paste0("../data_general/proc_data_Oz_fire_recovery/fit_kndvi_ttrDef5_preBS",Sys.time(),".parquet"))
