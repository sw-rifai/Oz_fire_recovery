...
tmp_ttr1 <- dat1[,`:=`(month=month(date))] %>% 
  .[,.(ttr2 = fn_min(.SD[days_since_fire>0][ndvi_anom_sd_3mo>0][month%in%c(6,7,8)]$days_since_fire) ),
    keyby=.(x,y,id)]
gc(full=TRUE)
...
gc(full=TRUE)
dat1 <- dat1[days_since_fire <= (ttr2+366)]
gc(full=TRUE)
...
write_parquet(dat1, "../data_general/proc_data_Oz_fire_recovery/fit_vi_ttrDef2_fire_train_dat.parquet")
...
write_parquet(dat2, "../data_general/proc_data_Oz_fire_recovery/fit_vi_ttrDef2_fire_test_dat.parquet")
